...
static void ota_task(void *args)
{
    ...
    /* step1. Fetch OTA information from server */
    ...
    /* step2. Parse firmware version from the firmware name in OTA upgrade URL, naming rule: app_pack_x.x.x.rbl where x.x.x is the version number */
    ...
    /* step3. Read OTA upgrade information from flash */
    ...
    /* If firmware_id in flash matches the one from server, it means OTA upgrade is complete and this is the device restart after upgrade */
    if (0 == strcmp(ota_version->firmware_id, ota_info.firmware_id)) {
        ...
        /* If OTA version recorded in flash matches current running software version, OTA upgrade succeeded; otherwise it failed */
        ...
        /* Report OTA upgrade result for the firmware_id */
        ...
        if (0 == agora_convoai_ota_result_report(&ota_report)) {
            /* If report succeeded, clear OTA upgrade info recorded in flash */
    ...
    /* step4. OTA upgrade */
    /* step4.1 Try to close convoai first, then perform OTA upgrade */
        app_event_send_msg(APP_EVT_CONVOAI_EXIT, 0);
        rtos_delay_milliseconds(1000);

    /* step4.2 Record this OTA upgrade info in flash */
    ...
    /* step4.3 If URL is https, replace with http */
...
/* Currently OTA upgrade is attempted only once after device powers on and successfully connects to network */
...
void agora_convoai_engine_start()
{
    ...
    /* If HTTP config fetch fails after device connects to network, need to fetch config first; if fetch still fails, exit */
