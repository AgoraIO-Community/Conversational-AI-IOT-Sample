...
/* Countdown duration for each ticket source (milliseconds) */
static const uint32_t s_ticket_durations[COUNTDOWN_TICKET_MAX] = {
    ...
    [COUNTDOWN_TICKET_OTA]    = COUNTDOWN_INFINITE,              // Pause countdown
};
...
void update_countdown(uint32_t s_active_tickets)
{
    ...
    // Check OTA pause ticket (highest priority)
    ...
    // Apply countdown arbitration result
