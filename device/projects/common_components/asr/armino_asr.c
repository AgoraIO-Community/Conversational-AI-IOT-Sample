...
static int wanson_asr_result_notify_handle(wanson_asr_handle_t wanson_asr, char *result, void *params)
{
    ...
    if (os_strcmp(result, "嗨阿古拉") == 0)                 // Recognized wake word: hi armino
    {
        LOGI("%s \n", "hi armino, cmd: 0 ");
        asr_result = 1;
    }
    else if (os_strcmp(result, "拜拜阿古拉") == 0)     // Recognized: byebye armino
    {
        LOGI("%s \n", "byebye armino, cmd: 0 ");
        asr_result = 2;
    }
    ...
    if (os_strcmp(result, "你好阿米诺") == 0)                 // Recognized wake word: nihao armino
    {
        LOGI("%s \n", "nihao armino, cmd: 0 ");
        asr_result = 1;
    }
