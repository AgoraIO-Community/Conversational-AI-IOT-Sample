...
typedef struct {
    int led1;          // First LED handle
    int led2;          // Second LED handle
    beken_timer_t timer; // Shared timer
    uint32_t interval;  // Alternate interval
    bool current_state; // Current dominant state
} AlternateGroup;
...
void led_vote_error(uint8_t led_handle, ErrorType err_type) {
    ...
    rtos_lock_mutex(&led_mutex);
    LedControlBlock* led = &led_pool[led_handle];
    // Increment corresponding error counter
    if (led->error_counts[err_type]++ == 0) {
        // First vote for this error type, may trigger state change
...
void led_resolve_error(uint8_t led_handle, ErrorType err_type) {
    ...
    if (led->error_counts[err_type] > 0 && --led->error_counts[err_type] == 0) {
        // This error type counter reaches zero, re-evaluate state
        update_active_error(led);
    }
